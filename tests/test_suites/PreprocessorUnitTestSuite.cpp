...
std::expected<std::vector<TokenPtr>, std::string> 
PreprocessorUnitTestSuite::TokenizeExpectedFile(const std::filesystem::path& file_path) {
...
std::string PreprocessorUnitTestSuite::BuildDetailedComparison(const std::vector<TokenPtr>& actual,
  ...
  for (size_t i = 0; i < max_size; ++i) {
    ...
    if (i >= actual.size() || i >= expected.size() || 
        !TokensEqual(actual[i], expected[i])) {
      ss << "  <--- DIFFERENCE";
    }
