...
std::vector<Param> ParseParamList(ContextParser& ctx, ITokenStream& ts) {
  ...
  while (true) {
    ...
    SkipTrivia(ts);
    if (ts.IsEof() || ts.Peek().GetLexeme() != ":") {
      ReportUnexpected(ctx.Diags(), "P_PARAM_COLON", "expected ':' after parameter name",
                       ts.TryPeek());
      break;
    }
    ts.Consume();
...
IState::StepResult StateTopDecl::TryStep(ContextParser& ctx, ITokenStream& ts) const {
  ...
  if (lex == "var" || lex == "val") {
    ...
    SkipTrivia(ts);
    if (ts.IsEof() || ts.Peek().GetLexeme() != ":") {
      ReportUnexpected(ctx.Diags(), "P_GLOBAL_VAR_COLON", "expected ':' after variable name",
                       ts.TryPeek());
      return std::unexpected(StateError("expected ':' after variable name"));
    }
    ts.Consume();
    ...
    SkipTrivia(ts);
    if (ts.IsEof() || ts.Peek().GetLexeme() != "=") {
      ReportUnexpected(ctx.Diags(), "P_GLOBAL_VAR_INIT", "expected '=' for global variable",
                       ts.TryPeek());
      return std::unexpected(StateError("expected '=' for global variable"));
    }
    ts.Consume();
    ...
    auto decl = ctx.Factory()->MakeGlobalVar(is_var, std::move(name), std::move(*type),
                                              std::move(init), span);
  ...
  const Token* tok = ts.TryPeek();
  ReportUnexpected(ctx.Diags(), "P_TOP_DECL", "expected top-level declaration", tok);
  if (tok != nullptr) {
    ts.Consume();
  }
  return true;
