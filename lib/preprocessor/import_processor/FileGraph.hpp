...
class FileGraph {
public:
  void AddDependency(const std::filesystem::path& from_path,
                     const std::filesystem::path& to_path);
  void Clear();

  [[nodiscard]] bool DetectCycles(
      std::vector<std::filesystem::path>& cycle_path) const;
  [[nodiscard]] std::expected<std::vector<std::filesystem::path>, CycleDetectedError>
