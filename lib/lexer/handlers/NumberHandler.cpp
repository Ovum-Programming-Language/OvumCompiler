...
OptToken NumberHandler::Scan(Lexer &lx) {
  ...
  if (first == '.') {
    ...
    return std::make_optional(TokenFactory::make_float_literal(std::move(raw), v, lx.GetLine(), lx.GetTokenCol()));
  }
  ...
  return std::make_optional(TokenFactory::make_int_literal(std::move(raw), vi, lx.GetLine(), lx.GetTokenCol()));
}
